...
public class CharacterScopePicker
{
    ...
    public bool Draw(string label, List<CharacterSearchScope> searchScopes)
    {
        ...
        using(var combo = ImRaii.Popup("scopePopup", ImGuiWindowFlags.Modal | ImGuiWindowFlags.NoSavedSettings))
        {
            if (combo)
            {
                ImGui.Text("Character Scope Editor".Loc());
                ...
                using (var child = ImRaii.Child("editor", new Vector2(0, 0), true, ImGuiWindowFlags.NoScrollbar))
                {
                    ...
                    {
                        ImGui.TextWrapped("The character scope editor allows you define which characters you want to search across.".Loc());
                        ImGui.TextWrapped("By default, every character Allagan Tools knows about is searched.".Loc());
                        ImGui.TextWrapped("By providing a set of scopes, you are narrowing down from which characters are displayed/counted.".Loc());
                    }
                    else
                    {
                        if (child)
                        {
                            ...
                            using (var commandBar = ImRaii.Child("commandBar", new Vector2(0, 24) * ImGui.GetIO().FontGlobalScale))
                            {
                                if (commandBar)
                                {
                                    ...
                                    ImGui.SameLine();
                                    if (ImGui.Button("Delete".Loc()) && _selectedScope != null)
                                    {
