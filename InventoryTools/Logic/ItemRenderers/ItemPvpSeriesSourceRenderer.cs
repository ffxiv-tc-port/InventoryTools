...
public class ItemPvpSeriesSourceRenderer : ItemInfoRenderer<ItemPVPSeriesSource>
{
    ...
    public override string SingularName { get; } = "PVP Series".Loc();
    public override string HelpText { get; } = "Is this item rewarded from a PVP series?".Loc();
    public override bool ShouldGroup { get; } = true;

    public override Action<ItemSource> DrawTooltip => source =>
    {
        var asSource = AsSource(source);
        ImGui.Text("Reward in PVP Series ".Loc() + asSource.PvpSeries.Value.RowId);
        ImGui.Text("Unlocks at level ".Loc() + asSource.Level);
    };

    public override Func<ItemSource, string> GetName => source =>
    {
        var asSource = AsSource(source);
        return "Series ?? Level ??".Loc(asSource.PvpSeries.Value.RowId, asSource.Level);
    };
