...
public class ItemGlamourReadySetSourceRenderer : ItemInfoRenderer<ItemGlamourReadySetSource>
{
    ...
    public override string SingularName => "Glamour Ready Set".Loc();
    public override string HelpText => "Is this item the glamour ready form of a 'Glamour Ready' outfit set?".Loc();
    ...
    {
        var asSource = AsSource(source);
        return "Contains ".Loc() + string.Join(", ", asSource.SetItems.Select(c => c.NameString));
    };
}