...
public class ItemSkybuilderInspectionUseRenderer : ItemSkybuilderInspectionSourceRenderer
{
    ...
    public override string HelpText => "Is the item the approved form of a item that must be inspected at the firmament?".Loc();
    public override Action<ItemSource> DrawTooltip => source =>
    {
        var asSource = AsSource(source);
        ImGui.Text("Reward: ??".Loc(asSource.Item.NameString));
        ImGui.Text("Required: ??".Loc(asSource.InspectionData.AmountRequired));
    };
}
...
public class ItemSkybuilderInspectionSourceRenderer : ItemInfoRenderer<ItemSkybuilderInspectionSource>
{
    ...
    public override string SingularName => "Sky Builder Inspection".Loc();
    public override string HelpText => "Can the item be inspected at the firmament to convert it into the approved form?".Loc();
    ...
    public override Action<ItemSource> DrawTooltip => source =>
    {
        ...
        ImGui.Text("Amount Required: ??".Loc(asSource.InspectionData.AmountRequired));
    };
