...
public class RetainerRetrieveOrderFilter : ChoiceFilter<RetainerRetrieveOrder>
{
    ...
    public override string Name { get; set; } = "Retainer Retrieve Order".Loc();
    public override string HelpText { get; set; } = "When displaying the items for a craft, if there are items to be retrieved should we display this before or after the shortfall is made up. If first is selected, it will make you retrieve items first, if last is selected, any missing items you'll need will have to be collected/purchased before the remainder will be shown for retrieval.".Loc();
